...
if(!category||category.items){return}this._getResource({url:`/api/data/${category.name}`,//'data/' + category.name + '.json',
onLoad(e){this.set("category.items",JSON.parse(e.target.responseText))},onError(e){this._setFailure(!0)}},attempts)}_getResource(rq,attempts){console.log(rq.url);let xhr=new XMLHttpRequest;xhr.addEventListener("load",rq.onLoad.bind(this));xhr.addEventListener("error",e=>{// Flaky connections might fail fetching resources
...
`}static get is(){return"shop-image"}static get properties(){return{alt:String,src:{type:String,observer:"_srcChanged"},placeholderImg:{type:String,observer:"_placeholderImgChanged"}}}_srcChanged(src){this.$.img.removeAttribute("src");this.$.img.style.transition="";this.$.img.style.opacity=0;/*if (src) {
                                    if(!src.includes("http"))
                                      this.$.img.src = "http://localhost:3000"+src;
                                    else*/this.$.img.src=src;//}
}_onImgLoad(){this.$.img.style.transition="0.5s opacity";this.$.img.style.opacity=1}_onImgError(){if(!this.placeholderImg){this.$.img.src="data:image/svg+xml,"+encodeURIComponent("<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 24 24\"><path fill=\"#CCC\" d=\"M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z\"/></svg>")}}_placeholderImgChanged(placeholder){this.style.backgroundImage="url('"+placeholder+"')"}}customElements.define(ShopImage.is,ShopImage);class ShopHome extends PolymerElement{static get template(){return html`
...
`}static get is(){return"shop-home"}static get properties(){return{categories:{type:Array},visible:{type:Boolean,observer:"_visibleChanged"}}}_visibleChanged(visible){if(visible){this.dispatchEvent(new CustomEvent("change-section",{bubbles:!0,composed:!0,detail:{title:"Home"}}))}}}customElements.define(ShopHome.is,ShopHome);window.performance&&performance.mark&&performance.mark("shop-app - before register");class ShopApp extends PolymerElement{static get template(){return html`
    ...
    <app-header role="navigation" id="header" effects="waterfall" condenses="" reveals="">
      <app-toolbar>
        ...
        <div class="logo" main-title=""><a href="/" aria-label="SHOP Home">TIENDA</a></div>
    ...
    <footer>
      <div class="demo-label">G6-AYD2</div>
    </footer>
...
if(detail.title){document.title=detail.title+" - TIENDA";this._announce(detail.title+", loaded");// Set open graph metadata
