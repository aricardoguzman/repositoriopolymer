define(["./shop-app.js"],function(_shopApp){"use strict";class ShopDetail extends _shopApp.PolymerElement{static get template(){return _shopApp.html`
    ...
    <div id="content" hidden$="[[failure]]">
      ...
      <div class="detail" has-content$="[[_isDefined(item)]]">
        ...
        <div class="pickers">
          <shop-select>
            <label id="sizeLabel" prefix>Tipo</label>
...
this.$.quantitySelect.value="1";this.$.sizeSelect.value="M";this.dispatchEvent(new CustomEvent("change-section",{bubbles:!0,composed:!0,detail:{category:item?item.category:"",title:item?item.title:"",description:item?item.description.substring(0,100):"",image:item?this.baseURI+item.image:""}}))})}}_unescapeText(text){let elem=document.createElement("textarea");elem.innerHTML=text;return elem.textContent}_formatPrice(price){return price?"Q"+price.toFixed(2):""}_addToCart(){// This event will be handled by shop-app.
this.dispatchEvent(new CustomEvent("add-cart-item",{bubbles:!0,composed:!0,detail:{item:this.item,quantity:parseInt(this.$.quantitySelect.value,10),type:this.$.sizeSelect.value}}))}_isDefined(item){return null!=item}_tryReconnect(){this.$.categoryData.refresh()}_offlineChanged(offline){if(!offline){this._tryReconnect()}}}customElements.define(ShopDetail.is,ShopDetail)});